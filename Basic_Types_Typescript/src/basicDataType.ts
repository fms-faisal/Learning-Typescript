...
// array
let friends: string[] = ["Rahim", "Karim"]; // array of strings
...
let eligibleRollList: number[] = [1, 2, 3]; // array of numbers
...
let mixedArray: Array<string | number> = [1, "two", 3];
mixedArray.push("four"); // valid because it allows both types
mixedArray.push(5); // valid as well

// Tuple
let address: [string, number, string] = ["Dhaka", 1, "Bashundhara"]; // fixed-length and fixed-type array
...
let ageName: (string | number)[] = [50, "Faisal"]; // array of mixed types (string and number) using union type
